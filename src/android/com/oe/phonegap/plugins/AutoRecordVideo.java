...
public class AutoRecordVideo extends CordovaPlugin {
  ...
  private abstract class AutoRecordVideoRunnable implements Runnable {
    public CallbackContext callbackContext;
    AutoRecordVideoRunnable(CallbackContext cb) {
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    ...
    if (ACTION_AVAILABLE_EVENT.equals(action)) {
      ...
      callbackContext.error("autoRecordVideo." + action + " is not a supported function. Did you mean '" + ACTION_SHARE_EVENT + "'?");
      return false;
    }
  }
  ...
  private boolean invokeEmailIntent(final CallbackContext callbackContext, final String message, final String subject, final JSONArray to, final JSONArray cc, final JSONArray bcc, final JSONArray files) throws JSONException {
    ...
    final AutoRecordVideo plugin = this;
    cordova.getThreadPool().execute(new AutoRecordVideoRunnable(callbackContext) {
      public void run() {
  ...
  private String getDownloadDir() throws IOException {
    final String dir = webView.getContext().getExternalFilesDir(null) + "/autorecordvideo-downloads"; // external

//    final String dir = webView.getContext().getCacheDir() + "/autorecordvideo-downloads"; // internal (no external permission needed)
  ...
  private Uri getFileUriAndSetType(Intent sendIntent, String dir, String image, String subject, int nthFile) throws IOException {
    ...
    if (image.startsWith("http") || image.startsWith("www/")) {
      ...
      if (image.startsWith("http")) {
        // filename optimisation taken from https://github.com/EddyVerbruggen/AutoRecordVideo-PhoneGap-Plugin/pull/56
